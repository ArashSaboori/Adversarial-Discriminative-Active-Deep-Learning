...
from dataloader import load, pad_zeros, get_init_indices, get_data
from ADADL import classifier, generator, discriminator1, discriminator2
...
'''make feature generator & feature discriminator'''
fea_g = generator()
fea_g.build(input_shape=(opt.BATCH_SIZE, opt.DIM_Z))
fea_g.summary()

fea_d1 = discriminator1()
fea_d1.build(input_shape=(opt.BATCH_SIZE, 17*17*60))
fea_d1.summary()

fea_d2 = discriminator2()
fea_d2.build(input_shape=(opt.BATCH_SIZE, 17*17*60))
fea_d2.summary()

d_loss, g_loss = dcgan_loss()
fea_g_optim = keras.optimizers.Adam(learning_rate=opt.GAN_LR, beta_1=0.9)
fea_d1_optim = keras.optimizers.Adam(learning_rate=opt.GAN_LR, beta_1=0.9)
fea_d2_optim = keras.optimizers.Adam(learning_rate=opt.GAN_LR, beta_2=0.95)
...
def test_func(te_data, te_label):
    ...
    k = te_label.size()[0]
    correct += te_pred.eq(te_label).cpu().sum()
    size += k
    ...
    with open(report, 'w') as f:
        ...
        f.write('\n')
        f.write("WINDOW_SIZE:{}, CHANNEL:{}, BATCH_SIZE:{}, BUDGET:{}".format(str(opt.WINDOW_SIZE),
...
def reports(data, label):
    ...
    if opt.DATASET == 'Botswana_May':
        targets = ['Water ', 'Floodplain ', 'Riparian ', 'Fire scar ','Island interior ',
                   'Woodlands ', 'Savanna ', 'Short mopane ', 'Exposed soil ']
                   
    elif opt.DATASET == 'Botswana_July':
        targets = ['Water ', 'Floodplain ', 'Riparian ', 'Fire scar ','Island interior ',
                   'Woodlands ', 'Savanna ', 'Short mopane ', 'Exposed soil ']
...
print('Source: {} to Target: {}  Accuracy : {}/{} ({:.2f}%) Max Accuracy : {}/{} ({:.2f}%) \n'.
              format(self.tr_set, self.te_set, 
...
if opt.ACQUISITION == 'al_acquisition':
    ...
    """---------------------------------------------------
    Train ADADL using intermediate features of initial labeled training data.
    """
    ...
    print("Start training ADADL using the feature of initial labeled training data!")
